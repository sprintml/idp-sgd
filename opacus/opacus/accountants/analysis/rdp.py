...
def get_privacy_spent(
    *, orders: Union[List[float], float], rdp: Union[List[float], float],
    delta: float, suppress_warning: Optional[bool] = None,
) -> Tuple[float, float]:
    ...
    if not suppress_warning:
        if idx_opt == 0 or idx_opt == len(eps) - 1:
            ...
            warnings.warn(
                f"Optimal order is the {extreme} alpha. Please consider "
                f"expanding the range of alphas to get a tighter privacy bound."
            )
